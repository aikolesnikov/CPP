...
int main()
{
    ...
    // 3.48. Пусть во входном потоке находится последовательность литер, заканчивающаяся точкой (кодировка ASCII):
    {
        ...
        {
            // int ci = 0;
            // while ( (ci<sN-1) && ((str[ci++]=getchar())!='\n') );
            // str[ci]='\0';

            fgets (str, sN, stdin);
            puts(str);
            printf("%s%u\n", "Size of string is: ", strlen(str));
...
3.49. Пусть во входном потоке находится последовательность литер, заканчивающаяся точкой (кодировка ASCII). Вывести в выходной поток последо-вательность литер, измененную следующим образом:
