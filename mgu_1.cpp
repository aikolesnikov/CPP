...
3.27. Дано натуральное число n. Найти значение числа, полученного следующим образом: из записи числа n выбросить цифры 0 и 5, оставив преж-ним порядок остальных цифр.
3.28. Дано натуральное число n. Получить все такие натуральные q, что n делится на q2 и не делится на q3.
3.29. Дано натуральное число n. Получить все его натуральные делите-ли.
3.30. Дано целое число m > 1.Получить наибольшее целое k, при кото-ром 4k < m.
...
3.36. Распечатать числа Фибоначчи (см. задачу 3.34), являющиеся про-стыми числами со значениями меньше n.
вательности { xi }при i → ∞
xi = (1+1/i)i, i = 1, 2, ...
