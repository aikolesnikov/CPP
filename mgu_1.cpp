...
int main()
{
    ...
    // Если уравнение имеет комплексные кор-ни, то распечатать их в виде v ± iw.
...
xi = (1+1/i)i, i = 1, 2, ...
Считать, что требуемая точность достигнута, если | xi-xi+1| < eps.3.38. Вычислить значение Σ i ! для i, изменяющихся от 1 до n. Восполь-зоваться соотношением Σ i ! = 1 + 1∗2 + 1∗2∗3 +...+ 1∗2∗3∗...∗n = 1+2∗(1+3∗(1+ +n∗(1)...)).
3.39. Пусть a0 и b0 - положительные вещественные числа. Соотноше-ниями an+1 = √(anbn) ; bn+1 = ( an+bn ) / 2 при n = 0, 1, 2, ... задаются две бесконеч-ные числовые последовательности {an}и {bn}, которые сходятся к общему пре-делу M(a0,b0), называемому арифметико-геометрическим средним чисел a0 и b0. Найти приближенное значение M(a0,b0) с точностью eps > 0. Поскольку при a0 < b0 ai < bi и, более того, a0 < a1 < ... < ai < ... bi < ... < b1 < b0 , то в качестве подходящего критерия прекращения вычислений можно использовать соотно-шение | ai - bi | < eps.
3.40. Вычислить квадратные корни вещественных чисел x = 2.0, 3.0, ... , 100.0. Распечатать значения x, √x, количество итераций, необходимых для вы-числения корня с точностью eps > 0.
...
3.41. Найти приближенное значение числа π с точностью eps > 0. Для этого можно использовать представление числа 2/π в виде произведения корней √(1/2) *√(1/2+1/2√(1/2))*√(1/2+ 1/2√(1/2+1/2√(1/2)))*... . Вычисления прекраща-ются, когда два следующих друг за другом приближения для числа π будут от-личаться меньше, чем на eps.
3.42. Для данного вещественного числа x и натурального n вычислить:
a) sin x + sin2x + ... + sinnx
b) sin x + sinx2 + ... + sinxn
c) sin x + sin(sin x ) + ... + sin ( sin (... sin(sin x) ... ))
3.43. Алгоритм Евклида нахождения наибольшего общего делителя (НОД) неотрицательных целых чисел основан на следующих свойствах этой ве-личины: пусть m и n - одновременно не равные нулю целые неотрицательные числа и m ≥ n. Тогда, если n = 0, то НОД(n, m) = m, а если n ≠ 0, то для чисел m, n, и r, где r - остаток от деления m на n, выполняется равенство НОД(m, n) = НОД(n, r). Используя алгоритм Евклида, определить наибольший общий дели-тель неотрицательных целых чисел a и b.
...
3.45. Натуральное число называется совершенным, если оно равно сум-ме всех своих делителей, за исключением самого себя. Дано натуральное чис- ло n. Получить все совершенные числа, меньшие n.
