...
//------------ Remont dorogi ---------------------
/*5. Генерируется массив на сто элементов в диапазоне от 0 до 20. Каждый элемент – это количество выбоин на участке 100 метров дороги. До 10-и выбоин –
хорошая дорога – рисуется символами *. Если плохой – символами  #, Если подряд встречаются 3 и более плохих участков, этот сектор отдается для ремонта бригаде со
спецтехникой – рисуется символом $ .  Нарисовать гистограмму. (верхний цикл  For, внутри  while – в нем обязательная проверка на выход за пределы массива.) Учимся
смотреть значения за пределами текущего. Например в массиве 5 10 11  6 12 13 14 16 3
Вывод:
1 5 *****
2 10 ##########
3 11 ###########
    ...
    for (int i=0; i<ArSize; ++i){
        ...
        if (Ar[i]<10){
            ...
            b_counter=0;
                ...
                while ( (b_counter < (ArSize-i)) && ( !((Ar[i+b_counter])<10))  )
                        b_counter++;
...
/*
//4. Написать функцию, которая принимает два массива одинакового размера и -  меняет их элементы так,
//чтобы первый массив стал инверсным вторым, а второй стал инверсным первым:
//Дано:                                                                  Результат:
//Массив1 – 1, 4, 5, 6, 2;                                Массив1 – 9, 7, 1, 6, 3;
//Массив2 – 3, 6, 1, 7, 9;                                Массив2 – 2, 6, 5, 4, 1.

    const int ArSize = rand()%20;
    int Ar1[ArSize], Ar2[ArSize];
    init_arr(Ar1, ArSize);
    init_arr(Ar2, ArSize);
    cout << "Array1: ";
    print_arr(Ar1,ArSize);
    cout << "Array2: ";
    print_arr(Ar2,ArSize);

    InverseTwoArrays(Ar1, Ar2, ArSize);
...
int n;
cout << "Input n for factorial: "; cin >> n;
cout << "n=" << n << "; Factorial " << n << "! = " << factorial(n) << endl;
*/
    return 0;
}
